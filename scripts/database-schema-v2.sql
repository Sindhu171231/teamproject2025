...
-- Create sellers table with proper structure
...
-- Sellers can manage their own profile
CREATE POLICY "Sellers can view own profile" ON public.sellers FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Sellers can update own profile" ON public.sellers FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "Anyone can view seller info" ON public.sellers FOR SELECT USING (true);
